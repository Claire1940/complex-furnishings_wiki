...
function matchScore(queryText: string, article: ArticleWithType, extraKeywords?: string[]): number {
  ...
  const strippedQuery = normalizedQuery.replace(/(lucid blocks?|complex furnishings)\s*/g, '').trim()
  const strippedTitle = normalizedTitle.replace(/(lucid blocks?|complex furnishings)\s*/g, '').trim()
